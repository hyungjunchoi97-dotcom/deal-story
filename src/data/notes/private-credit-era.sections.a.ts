...
export const SECTIONS_PART_A: NoteSection[] = [
  ...
  // ════════════════════════════════════════════════════════════════
  {
    heading: "1. 17년 호황의 끝자락 — 세 가지 신호가 한 분기에 겹쳤다",
    headingEn: "1. The End of a 17-Year Bull — Three Signals in One Quarter",
    blocks: [
      {
        type: "text",
        body:
          "2025년 봄, 사모대출(Private Credit, 이하 PC) 시장에서 *세 가지 사건*이 한 분기에 겹쳤다. 어떤 하나도 결정적이지 않다. 그러나 세 개가 함께 일어났다는 사실이 의미를 만든다.\n\n첫째, 2025년 3월 4일 — 한국 대형마트 *홈플러스*가 서울회생법원에 법인회생을 신청했다. MBK파트너스가 2015년 ₩7.2조에 인수한 지 10년 만이다. 인수 당시 차입금 약 ₩4.3조(전체의 59.8%)가 인수금융으로 조달됐고, 10년이 지난 시점에 누적 부채는 ₩8.5조 규모로 보도됐다.¹ 한국 PEF(Private Equity Fund, 사모투자펀드)·인수금융 역사상 *단일 최대 손실 사례*가 될 가능성이 열렸다.\n\n둘째, 같은 봄 미국에서는 *Blue Owl Capital*의 비상장 BDC(Business Development Company, 사업개발회사) — OBDC II — 가 일부 자산을 할인 매각한다는 보도가 나왔다. 비공개 자산을 *시장에 던지는* 행동 자체가 흔치 않다. 인터벌 펀드·non-traded BDC가 환매 한도(분기 2% / 연 5%)에 부딪히기 시작했다는 첫 시그널이었다.²\n\n셋째, 2025년 9월 *Apollo Global Management*의 CEO Marc Rowan은 CNBC 인터뷰에서 \"전통적 투자 모델은 깨졌다\"고 말했다. 같은 인터뷰에서 그는 \"부채의 20-30%만 은행 balance sheet에 있고, 나머지는 investment marketplace에 있다\"고 했다. 다른 자리에서는 *shakeout* (구조조정) 이라는 단어를 썼다.³\n\n한 분기 안에 *한국 PEF 최대 손실 사례*, *미국 retail PC의 환매 시그널*, *PC 시장 원조 운용사의 shakeout 발언*이 겹쳤다. 그래서 우리는 이 글을 쓴다.",
        bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "본론에 들어가기 전에 *PC가 무엇인지*를 명확히 짚어둔다. 영어 약어를 모르면 뒤의 어떤 표도 의미가 없다.\n\nPC를 한 줄로 정의하면 — *공개시장(은행 대출·공모채)을 거치지 않고, 자산운용사가 직접 모집·실행하는 기업·자산 담보 대출* 이다. 차주는 주로 중견·중소기업 또는 PE 스폰서가 인수한 회사이고, 대주는 BDC·PC 펀드·보험사 GA(General Account, 일반계정)다.",
        bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "은행 대출은 *백화점 매대*다. 진열대가 정해져 있고, 약관도 표준화돼 있고, 누구나 볼 수 있다. PC는 *동네 부자 아저씨의 직접 대출*에 가깝다. 조건은 상대방을 보고 협상하고, 약관은 거래마다 다시 쓴다. 영어권 비유로 옮기면 *the wealthy uncle down the street, lending out of his own pocket* — 한 사람이 직접 본다는 점이 핵심이다.",
          bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "BSL(Broadly Syndicated Loan, 광범위 신디케이트 대출)은 *공동구매*다. JP Morgan이 한 거래를 주선하면 50-100개 기관이 조각으로 나눠 산다. PC는 *맞춤 정장*이다. Blue Owl이 단독 또는 3-4개 club deal로 한 회사를 끝까지 hold한다. 영어로 적으면 BSL = *Costco bulk pack*, PC = *bespoke tailor*. 가격·약관·hold 기간이 전부 다르다.",
          bodyEn:
      ...
      },
      {
        ...
        table: {
          ...
          highlightRows: [5],
          caption: "Sources: Proskauer (PC vs BSL overview), CreditSights, KBRA DLD. Highlight = valuation timing — 이 한 줄이 \"왜 위기 때 진짜 노출이 늦게 보이는가\"의 답이다.",
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "*PC AUM* 자체가 source마다 다르다. Preqin 협의 정의는 2024년 $1.7T, BIS 광의 정의는 $2.1T, JPM·PitchBook 보수 정의는 $1.2T. 본문에서 단일 숫자를 인용할 땐 *Preqin 협의 기준*으로 통일했다. 다른 소스를 인용할 땐 명시한다.",
          bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "PC AUM은 2008년 GFC(Global Financial Crisis, 글로벌 금융위기) 직후 약 $250B에서 2024년 약 $1.7T로, 17년간 *약 7배* 증가했다 (Preqin 협의 정의). 같은 기간 미국 GDP는 약 1.8배, S&P 500은 약 4배 올랐다. PC는 *어떤 다른 자산군보다도 빠르게* 늘었다.⁴\n\n2024년 한 해 deployed capital(실집행 자본)은 약 $592.8B로 직전년 $333.4B 대비 +78%. Preqin은 2030년 $2.64T를 전망하고, BlackRock은 HPS 인수 직후 자료에서 최대 $4.5T 시나리오를 제시했다.⁵",
        bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "*$1.7T의 절반 가까이*는 보험사 GA에서 왔다. Apollo가 Athene을 통해, KKR이 Global Atlantic 100% 인수를 통해, Brookfield가 American Equity Life를 통해, Blackstone이 Corebridge·Allstate·Resolution Life SMA(Separately Managed Account)를 통해 — *연금 부채(평균 듀레이션 7-10년)*를 long-dated PC 자산과 매칭했다. 즉 PC 성장의 자금원은 *retail이 아니라 보험·연금 long money*가 본체다. (→ ✓11)",
          bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "PC가 성장한 이유를 한 줄로 답하면 — *은행이 떠났기 때문*이다. 그러나 은행은 자발적으로 떠난 게 아니다. *규제가 떠밀었다*.\n\n첫 번째 사슬은 Basel III. BIS(Bank for International Settlements, 국제결제은행) Basel 위원회가 2010년 발표한 규제다. 핵심은 *자본 요건 강화*와 *자본보존완충(Capital Conservation Buffer, CCB) 신설*. 2013년부터 2019년까지 7년에 걸쳐 phase-in 했다. 은행이 위험가중자산 대비 보유해야 할 자본 비율이 8.0%에서 10.5%로 올라갔고, G-SIB(Global Systemically Important Bank, 글로벌 시스템적 중요 은행)는 추가 1-3.5% buffer가 더해졌다.⁶\n\n두 번째 사슬은 미국 *Interagency Leveraged Lending Guidance* (LLG). OCC(통화감독청)·Fed(연준)·FDIC(연방예금보험공사) 세 기관이 2013년 3월 22일 공동 발표한 가이던스다. 핵심 룰은 단순했다 — *총부채 / EBITDA > 6배 거래는 추가 scrutiny 대상*. 적용 대상은 미국 은행뿐, PC 펀드는 비적용. 결과는 더 단순했다. *6배+ 거래가 통째로 PC로 넘어왔다.* PC는 그 자리에서 7-9배 leverage 거래를 흡수했다.⁷\n\n세 번째 사슬은 *Volcker Rule* (Dodd-Frank §619). 2015년 시행. 은행 내부의 PE 펀드 sponsorship과 자기자본 거래(proprietary trading)를 제한했다. 은행 내부에서 키우던 PC 팀들이 *밖으로 나가* 독립 운용사가 됐다. HPS의 분사 경로(2007년 JPMorgan AM 산하 Highbridge → 2016년 분사 → 2025년 BlackRock 인수)가 정확히 이 패턴이다.⁸",
        bodyEn:
      ...
      },
      {
        ...
        table: {
          ...
          highlightRows: [6],
          caption: "Source: BIS Basel III phase-in arrangements. G-SIB은 추가 1-3.5% buffer. *2019년 최종 단계*에서 은행은 자본을 모으느라 변두리 leverage 거래를 포기했다.",
      ...
      },
      {
        ...
        chart: {
          ...
          caption: "Source: PitchBook LCD, Proskauer, JPM 종합 (중소·중견 LBO 한정). 메가딜(>$3B)은 여전히 BSL 우세. 2022년 PC 비중 60%는 *금리 급등으로 BSL 시장이 stall한 결과*다 — 일시 효과.",
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "2025년 12월 5일, OCC와 FDIC가 2013 Interagency Leveraged Lending Guidance에서 *withdraw*했다 (트럼프 2기 deregulation, Fed는 잔류). 12년간 PC를 키운 규제 한 축이 끊겼다. 단기 효과는 *은행이 6배+ 거래로 다시 들어올 수 있다*는 것 — PC와 *직접 경쟁* 가능성. 이 단 한 줄이 2026년 PC 가격 압박의 시작점이다.⁹",
          bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "PC AUM의 ~44%가 Direct Lending이다. 우리가 *PC*라고 부를 때 머릿속에 떠올리는 그림 — *Blue Owl이 Carlyle이 인수한 회사에 $2B unitranche를 빌려준다* — 가 정확히 이것이다.\n\nDirect Lending의 표준 구조는 *Unitranche*. 1st lien(1순위 담보)과 2nd lien(2순위 담보)을 합쳐 단일 트랜치로 만든 대출이다. 차주 입장에서는 협상 상대가 한 곳, 변제 우선순위 분쟁이 없다는 장점이 있다. 대주 입장에서는 더 높은 금리(SOFR + 500-700bp)와 더 많은 컨트롤을 가져간다.\n\n2024년 미국 direct lending 거래 volume은 약 $302B로 전년 대비 +107%, 중기업(Middle Market) direct lending은 약 $139B로 +85%. 전체 시장 volume은 약 $390B로 두 배가 됐다. LBO 평균 leverage는 2024년 Q4 기준 4.8배 EBITDA였다.¹⁰",
        bodyEn:
      ...
      },
      {
        type: "text",
        body:
          "### Cov-Lite — \"covenant이 없는 약관\"\n\nCov-Lite(Covenant-Lite, 약식 약관)는 차주에게 유리한 약관 구조다. 전통적 maintenance covenant(정기 재무비율 준수 조항)가 *완화되거나 사라진* 대출. 위반 trigger가 없거나 매우 느슨해서 차주가 EBITDA add-back을 늘리거나 PIK(Payment-in-Kind, 이자 원금 가산) 옵션을 자유롭게 쓸 수 있다.\n\n전체 institutional leveraged loan 시장의 *93%*가 Cov-Lite. Upper middle market PC(EBITDA $50M+)도 약 30%, 메가딜(>$500M)은 50%가 maintenance covenant 없음. 일반 PC 평균은 2023년 ~4%에서 2024-25년 ~21%로 *5배 급등* (Proskauer 측정).¹¹\n\n### PIK — \"이자를 현금이 아니라 원금에 얹는다\"\n\nPIK(Payment-in-Kind)는 차주가 이자를 현금으로 내는 대신 *원금에 가산*해 누적시키는 구조다. 단기 현금흐름이 나쁜 회사에 유리하지만, 대주 입장에서는 *실현 수익이 늦어진다*. 2024-25년 PC 포트폴리오의 PIK 비중 상승은 BDC 환매 압력의 한 요인.\n\n### EBITDA Add-back — \"미래 비용 절감을 미리 더한다\"\n\nLBO 대출에서 가장 논쟁적인 항목. *예상 시너지*, *비반복 비용*, *cost-out 계획*을 EBITDA에 더해 leverage 비율을 낮춰 보이게 만든다. 2024년 일부 거래에서는 add-back이 reported EBITDA의 30-40%까지 올라갔다는 보도가 있다.\n\n### SOFR — \"LIBOR 사망 후 표준 변동금리\"\n\nSOFR(Secured Overnight Financing Rate)는 LIBOR 사망(2021-23 phase-out) 후 미국 변동금리 시장의 표준. PC 대출의 대부분이 *SOFR + 500-700bp spread* 형태. 금리가 오르면 PC 수익률이 자동 인상되지만 동시에 차주 이자부담도 자동 인상된다 — *2022-23년 금리 급등*이 PC borrower interest coverage ratio를 1 아래로 떨어뜨린 직접적 원인.",
        bodyEn:
      ...
      },
      {
        ...
        table: {
          ...
          highlightRows: [0],
          caption: "Sources: LSTA 2024 Direct Lending Review, Proskauer. Highlight = Finastra: *최초로 PC 컨소시엄이 메가딜 BSL 시장을 직접 대체*한 사건.",
    ...
    blocks: [
      {
        type: "text",
        body:
          "Distressed Credit는 PC AUM의 ~11%로, Direct Lending에 비하면 작다. 그러나 *역사적으로 가장 안정적인 수익률*을 낸 영역이다. 1986-2024년 미국 HY(High Yield, 고수익) 채권의 연환산 수익률은 7.83%, 10년 국채 5.14%, HY 평균 디폴트율은 3.5%였다 (Oaktree 자체 정리).¹²\n\n이 영역의 대표 플레이어는 *Oaktree Capital Management* (Brookfield 산하). 공동 창업자 Howard Marks는 1995년 회사 출범 이후 *credit cycle*에 대한 가장 영향력 있는 메모를 써온 인물이다. 그의 2024-25년 메모 두 편이 PC 전체 논쟁의 가장 정직한 framing을 제공한다.",
        bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "\"오늘 신용투자는 매력적이다. 고정수익을 5% + 추가 spread로 가져갈 수 있다. 다만 — *yield spread는 이유가 있어서* 존재한다. 무위험 자산 대비 약 200-400bp의 추가 수익은 *디폴트 위험과 illiquidity 위험에 대한 정직한 보상*이다. 누군가가 이 spread를 '과도하다'고 한다면, 그는 진짜 위험을 측정하지 못한 것이다.\"\n\n*— 본 메모는 Oaktree의 매수 권유로도 읽히지만, 동시에 \"credit이 cheap하지는 않다\"는 정직한 인정이기도 하다.*",
          bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "\"Direct lending이 *소프트웨어 LBO에 집중*된 점이 우려된다. ~20x EBITDA로 인수된 회사들 — 이는 미래 EBITDA 성장이 *반드시 일어나야만* 정당화되는 가격이다. AI가 소프트웨어 가치를 disrupt 한다면 — 그리고 그 가능성은 결코 0이 아닌데 — PC 채권자가 가장 먼저 손실을 본다.\"\n\n*— Marks는 systemic risk라 부르지 않는다. concentrated risk라고 부른다. 차이가 중요하다.*",
          bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "\"credit carelessness는 있다. 그러나 systemic은 아니다.\"\n\n— 이 한 문장이 Marks의 *최종 입장*이다. 2024년 \"Gimme Credit\"의 buy-case와 2025년 software LBO 우려를 양손에 들고, 결론은 \"방심은 보이지만 시스템은 안전하다\"로 정리됐다. PC 시장의 가장 영향력 있는 운용사가 *경계는 하되 panic하지 않는다*는 입장을 명확히 한 셈이다.¹³",
          bodyEn:
      ...
      },
      {
        type: "text",
        body:
          "Distressed에서 PC와 가장 가까운 두 가지 실행 도구를 정리한다.\n\n*DIP financing* (Debtor-in-Possession, 채무자유지대출)은 챕터 11 진입 기업에 신규로 빌려주는 대출이다. 도산 절차 안에서 *최우선 변제* 지위를 받는다. 2024년 *WeWork DIP* (Goldman Sachs 주선, Cupar Grimmond 등)가 대표 사례. PC 운용사 입장에서는 다른 채권자가 다 손실을 본 다음에도 자기 자본은 회수할 수 있는 *가장 안전한 distressed 도구*다.\n\n*Loan-to-Own*은 부실 회사의 채권을 시장에서 매입한 다음 *debt-for-equity swap*으로 지분을 확보하는 전략이다. 2024년 Vista의 *Pluralsight*가 정확히 이 패턴이었다 — Blue Owl·Ares·Golub·Oaktree·Benefit Street·Goldman·BlackRock이 채권자 컨소시엄으로 회사를 인수했다. PE가 손실을 보고, PC가 회사를 갖는다. (→ ✓13)",
        bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "Mezzanine은 senior debt와 equity 사이의 *후순위 자본*이다. Cash interest + PIK 합쳐 yield 12-15%, equity warrant(주식 인수권)를 동반하는 경우가 많다. 과거 미국 LBO의 25%를 차지했으나 unitranche 부상으로 현재 PC AUM의 ~10% 수준까지 떨어졌다.¹⁴\n\nMezz가 줄어든 이유는 *unitranche가 더 단순하기 때문*이다. 차주가 senior + mezz 두 트랜치를 협상하는 대신 unitranche 하나로 모든 걸 끝낸다. mezz 운용사들은 점차 *junior 포지션*과 *growth financing*으로 이동했다 — 좀 더 equity-like한 리스크.",
        bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "ABF(Asset-Based Finance)는 *특정 자산 풀*을 담보·기초자산으로 구조화한 대출이다. 자동차 리스, 항공기 리스, 매출채권, 소비자 채권, 부동산 임대료, 로열티 — 회사 자체의 신용이 아니라 *자산의 현금흐름*에 기반한다.\n\nApollo의 추산에 따르면 광의 ABF 시장 규모는 약 $5-6T (이 중 PC 펀드 운용분은 약 $300-400B). Apollo는 자체 분류상 \"Direct Origination + ABF + Opportunistic + Multi-credit\" 합계 $749.2B (2024 Q4, IR). Marc Rowan은 ABF 시장이 2030년 *$40T*까지 성장한다는 자체 outlook을 여러 차례 제시했다 — *매우 공격적*인 숫자다.¹⁵",
        bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "NAV Finance는 *PE 펀드의 NAV(보유 portfolio 자산 가치)를 담보로 한 fund-level 대출*이다. 약정금(uncalled capital)이 아닌 *이미 투자된 자본*에 대한 leverage라는 점이 핵심이다.\n\n2024년 평균 per-lender deal volume이 €800M+, 전년 €330M 대비 +142% (Rede Partners 2025 survey). 17Capital은 2025년 deployment ~$70B, 2030년 ~$145B 전망을 제시했다 — TAM(Total Addressable Market)은 ~$700B 추정.¹⁶\n\n표준 LTV(Loan-to-Value)는 NAV 대비 10-25%. covenant는 보통 \"NAV가 loan balance의 3배 이상 유지\". 선두 플레이어는 17Capital(Brookfield 산하), Pemberton, Whitehorse Liquidity Partners, Ares NAV Lending, Hark Capital, Goldman PE Financing, KKR Capital Markets, Blackstone GSO.",
        bodyEn:
      ...
      },
      {
        ...
        callout: {
          ...
          body:
            "Vista Equity Partners는 Vista Outcomes Fund의 NAV를 담보로 대출을 받아 *LP에게 배당(dividend recap)을 실행*했다. 이미 손실 위험이 있는 portfolio에 leverage를 추가해 *LP 입장에서 risk-adjusted return을 악화*시킨다는 비판이 거셌다. ILPA(Institutional Limited Partners Association, 기관 LP 협회)는 2024년 NAV financing 가이드라인을 발간하며 *LP 사전 동의 필수*를 권고했다. 이 한 사건이 NAV finance 전체에 *LP 불신*이라는 그림자를 남겼다.¹⁷",
          bodyEn:
    ...
    blocks: [
      {
        type: "text",
        body:
          "CLO(Collateralized Loan Obligation, 대출담보부증권)는 *BSL 시장*의 일부지만 일부 PC 운용사가 CLO equity tranche를 PC 펀드에 편입한다. CLO를 *6층짜리 빌라*에 비유하면 — 1층(AAA)부터 6층(equity)까지 손실 부담 순서가 정해진다. 6층(equity)이 *맨 먼저 손실을 흡수*하는 대신 *가장 높은 수익률*을 가져간다. 잘 굴러갈 때는 연 15-20% IRR, 디폴트가 늘면 0이 될 수도 있다.\n\n2024년 미국 CLO 신규 발행은 약 $200B (LSTA). refinancing·reset 활동이 활발했다. Apollo·Blackstone·Ares·KKR 모두 CLO 운용 라이선스를 보유한다.¹⁸",
        bodyEn:
