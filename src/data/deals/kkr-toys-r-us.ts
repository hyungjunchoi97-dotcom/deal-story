...
const deal: DealData = {
  ...
  levfinOverview: {
    ...
    body: "토이저러스 LBO에서 부채 구조를 해부하면 레버리지드 파이낸스의 가장 무서운 교훈이 드러난다. $53억 부채, 연 $400M+ 이자, 그리고 그 앞에서 무력해진 디지털 투자 예산. TLB 대주단은 '선순위'였지만 회수율은 $0.20-0.25에 그쳤고, PE는 에쿼티 $13억을 전액 잃었다. 이 딜이 LevFin 신용 애널리스트들이 '소매업 레버리지 임계치'를 논할 때마다 꺼내드는 이유다.",
    tranches: [
      {
    ...
    lessons: [
      {
        ...
        body: "소매업은 마진이 낮고(EBITDA 마진 7-8%) 경기 변동에 민감하다. 6× 이상 레버리지에서 이자 부담이 EBITDA의 60%+ 를 잡아먹으면, 새로운 경쟁자(아마존)가 출현할 때 대응 투자가 불가능해진다. 토이저러스의 신용 애널리스트가 '업종 조정 레버리지 임계치'를 확인했어야 했다.",
      },
      {
        icon: "💸",
