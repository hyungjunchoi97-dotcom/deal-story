...
const accent = "#3b82f6";
// accentLight available for future use: "rgb(239 246 255)"
const accentDark = "#1d4ed8";

// ── Chart Data ────────────────────────────────────────────────────────────────
const priceData = [
  { date: "Sep 2017", price: 100 },
  { date: "Jan 2018", price: 105 },
  { date: "Jul 2018", price: 112 },
  { date: "Jan 2019", price: 120 },
  { date: "Jul 2019", price: 148 },
  { date: "Jan 2020", price: 165 },
  { date: "Mar 2020", price: 145 },
  { date: "Jul 2020", price: 192 },
  { date: "Jan 2021", price: 215 },
  { date: "Jun 2021", price: 230 },
  { date: "Jan 2022", price: 185 },
  { date: "Jun 2022", price: 96 },
  { date: "Oct 2022", price: 46 },
  { date: "Dec 2023", price: 58 },
];

const durationData = [
  { bond: "2yr", duration: 2.0, fill: "#10b981" },
  { bond: "10yr", duration: 9.2, fill: accent },
  { bond: "30yr", duration: 19.5, fill: "#f59e0b" },
  { bond: "100yr\n(AT)", duration: 47, fill: "#ef4444" },
];
...
const ecbRateData = [
  ...
  { date: "2020", rate: -0.5 },
  { date: "2021", rate: -0.5 },
  { date: "Jul 2022", rate: 0.5 },
  { date: "Oct 2022", rate: 1.5 },
  { date: "Feb 2023", rate: 2.5 },
  { date: "Sep 2023", rate: 4.0 },
  { date: "Jun 2024", rate: 3.75 },
];
      ...
      <div className="p-5 sm:p-8">
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={priceData} margin={{ top: 8, right: 16, left: 8, bottom: 4 }}>
            <defs>
              <linearGradient id="priceGradAT" x1="0" y1="0" x2="0" y2="1">
            ...
            <ReferenceLine
              ...
              stroke="#10b981"
              strokeDasharray="4 3"
              label={{ value: ko ? "고점 230" : "Peak 230", position: "insideTopLeft", fontSize: 9, fill: "#10b981" }}
            />
            <ReferenceLine
              x="Oct 2022"
              stroke="#ef4444"
              strokeDasharray="4 3"
              label={{ value: ko ? "저점 46" : "Trough 46", position: "insideTopRight", fontSize: 9, fill: "#ef4444" }}
            />
            <Area
              type="monotone"
              dataKey="price"
        ...
        >
          {[
            { val: "100", label: ko ? "발행가 (2017)" : "Issue price (2017)", color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20" },
            { val: "230", label: ko ? "고점 (Jun 2021)" : "Peak (Jun 2021)", color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
            { val: "46", label: ko ? "저점 (Oct 2022)" : "Trough (Oct 2022)", color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-900/20" },
          ].map((item) => (
            <motion.div key={item.val} variants={fadeUp()} className={`rounded-xl p-3 text-center ${item.bg}`}>
...
function DurationChart({ ko }: { ko: boolean }) {
  const data = durationData.map((d) => ({
    ...d,
    bond: ko
      ? d.bond.replace("2yr", "2년물").replace("10yr", "10년물").replace("30yr", "30년물").replace("100yr\n(AT)", "100년물\n(오스트리아)")
      : d.bond,
  }));
  return (
    <motion.div
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data} margin={{ top: 8, right: 16, left: 8, bottom: 20 }}>
            ...
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              tickFormatter={(v) => `${v}${ko ? "년" : "yr"}`}
            />
            <Tooltip
              formatter={(v) => [`${v}${ko ? "년" : " years"}`, ko ? "수정 듀레이션" : "Modified Duration"]}
              contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff" }}
            />
...
// ── ECB Rate Chart ────────────────────────────────────────────────────────────
function EcbRateChart({ ko }: { ko: boolean }) {
  return (
    <motion.div
      variants={fadeUp(0.1)}
      className="mt-8 rounded-2xl overflow-hidden border border-gray-200/60 dark:border-gray-700/60"
    >
      <div className="bg-gray-50 dark:bg-gray-800/60 px-5 py-3 border-b border-gray-200/60 dark:border-gray-700/60">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
          {ko ? "ECB 기준금리 추이 (%) — 2017~2024" : "ECB Base Rate (%) — 2017–2024"}
        </p>
      </div>
      <div className="p-5 sm:p-8">
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={ecbRateData} margin={{ top: 8, right: 16, left: 8, bottom: 4 }}>
            <defs>
            ...
            />
            <Tooltip
              formatter={(v) => [`${v}%`, ko ? "ECB 금리" : "ECB Rate"]}
              contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff" }}
            />
            <ReferenceLine y={0} stroke="#9ca3af" strokeDasharray="3 2" strokeWidth={1} />
            <ReferenceLine
              x="Jul 2022"
              stroke="#ef4444"
              strokeDasharray="4 3"
              label={{ value: ko ? "인상 개시" : "Hike Start", position: "insideTopRight", fontSize: 9, fill: "#ef4444" }}
            />
            <Area
              type="monotone"
              dataKey="rate"
...
function DurationMathCallout({ ko }: { ko: boolean }) {
  return (
    <motion.div
      variants={fadeUp(0.1)}
      className="mt-8 rounded-2xl overflow-hidden border-2 border-blue-200 dark:border-blue-700"
    >
      <div className="bg-blue-50 dark:bg-blue-900/20 px-5 py-3 border-b border-blue-200 dark:border-blue-700">
        <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">
          {ko ? "듀레이션 수학 — 왜 230→46이 가능했나" : "Duration Mathematics — Why 230→46 Was Possible"}
        </p>
      </div>
      <div className="p-5 sm:p-6 bg-white dark:bg-gray-900">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={VP} className="space-y-3">
          {[
            {
              label: ko ? "수정 듀레이션" : "Modified Duration",
              value: "~47년 / years",
              sub: ko ? "금리 1%p = 가격 47% 변화" : "1% yield change = 47% price change",
              color: "text-blue-700 dark:text-blue-300",
              bg: "bg-blue-50 dark:bg-blue-900/20",
            },
            {
              label: ko ? "2020년 금리 하락" : "2020 Rate Decline",
              value: "−1.5%p",
              sub: ko ? "가격 이론치 +70.5%, 실제 +130% (볼록성 효과)" : "Theoretical price gain +70.5%, actual +130% (convexity effect)",
              color: "text-emerald-700 dark:text-emerald-300",
              bg: "bg-emerald-50 dark:bg-emerald-900/20",
            },
            {
              label: ko ? "2022년 금리 상승" : "2022 Rate Rise",
              value: "+2.0%p",
              sub: ko ? "가격 이론치 −94%, 실제 −80% (볼록성 완충)" : "Theoretical price loss −94%, actual −80% (convexity buffer)",
              color: "text-red-700 dark:text-red-300",
              bg: "bg-red-50 dark:bg-red-900/20",
            },
          ].map((item, i) => (
            <motion.div key={i} variants={fadeUp(i * 0.07)} className={`rounded-xl px-4 py-3 ${item.bg}`}>
              <div className="flex items-center justify-between gap-3">
                <span className="text-[11px] text-gray-500 dark:text-gray-400">{item.label}</span>
                <span className={`text-[14px] font-black ${item.color}`}>{item.value}</span>
              </div>
              <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{item.sub}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
function StatsCallout({ ko }: { ko: boolean }) {
  const stats = [
    {
      val: "5.7x",
      label: ko ? "초과청약 (€115억 수요)" : "Oversubscribed (€11.5B demand)",
    ...
    {
      val: "+130%",
      label: ko ? "2020년 고점 가격 상승 (발행가 대비)" : "2020 peak price gain vs. issue price",
    ...
    {
      val: "−80%",
      label: ko ? "2022년 고점 대비 가격 하락" : "2022 price decline from peak",
...
// ── Main Export ───────────────────────────────────────────────────────────────
export default function Austria100yrClient({
  deal,
  lang,
}: {
  deal: MarketDeal;
  ...
  function getVisual(i: number) {
    const visuals = [
      ...
      <DurationChart key="1" ko={ko} />,
      <EcbRateChart key="2" ko={ko} />,
      <DurationMathCallout key="3" ko={ko} />,
      null,
    ];
