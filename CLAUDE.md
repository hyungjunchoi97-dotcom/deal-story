...
## 10. Daily Blog Tone (market 카테고리 전용)

**확정: 2026-06-05 (chip-crash 노트)**

`category: "market"` 데일리 블로그는 §9와 *다른 톤*을 쓴다. 빠른 호흡, 5-10분, 시의성 중심.
...
| em-dash | `—` 금지 → 콤마나 풀어쓰기 |
| 화살표 | `→` 단계, 인과에 자유 |
| 시장 인용 | 큰따옴표 `"~"` 직접 인용 (시장 분위기 표현) |
| 섹션 헤더 | `Why 하락? 1.` 같은 캐주얼 헤더 허용 |
| 결론 | 끝에 "한 문장으로 정리하면" |
| 차트 | `daily-bar` 범용 막대 차트 재사용 (DailyBarPoint: label/value/highlight/note). 데이터만 갈아끼움 |
| 구성 | 후크 → Why(원인) → 한국 변수 → 투자자 takeaway → 체크포인트 → 한 줄 정리 |
| takeaway | 단순 사실 나열 금지. "조정이냐 전환이냐 + 뭘 봐야 아냐"까지 줄 것 |
| 영문판 | bodyEn 등 평행 작성 (i18n strict 통과 필요) |

데일리 블로그는 *투자자가 딱 얻어가는 게 있어야* 한다. 위기/평시 프레임을 정확히 구분하고(예: 강세장 속 조정 vs 위기), 과장된 역사 비교는 피한다.
